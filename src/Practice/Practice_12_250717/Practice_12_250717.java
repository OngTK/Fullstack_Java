...
import Practice.Practice_10_250710.Member;
...
//        3.main 함수에서 Shape shape = new Circle(); 코드를 작성한 뒤, shape.draw()를 호출했을 때 어떤 결과가 나오는지 확인하고 그 이유를 주석으로 설명하세요.
        ...
        // 상속관계에 두 클래스에 동일한 명칭의 메소드가 있을 경우, 객체가 생성되었을 당시의 타입의 메소드를 우선으로 실행함
        // 변수 shape의 경우 타입은 Shape 이다. 다만, shape에 대입된 new Circle()의 타입은 Circle 이므로, shape의 태생 타입은 circle
        // 따라서, shape.draw(); 실행 시 태생 타입에 따라서 Circle클래스의 draw 메소드인 "원을 그립니다."가 실행됨
